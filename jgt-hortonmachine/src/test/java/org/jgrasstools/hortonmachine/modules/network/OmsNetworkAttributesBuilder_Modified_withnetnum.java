...
import org.jgrasstools.hortonmachine.modules.network.networkattributes.CopyOfNetworkChannel;
...
public class OmsNetworkAttributesBuilder_Modified_withnetnum extends JGTModel {
    ...
    private List<CopyOfNetworkChannel> channels;
    ...
    public void process() throws Exception {
        ...
        String hackName = CopyOfNetworkChannel.HACKNAME;
        b.add(hackName, Integer.class);
        String netnumName = CopyOfNetworkChannel.NETNUMNAME;
        b.add(netnumName, Integer.class);
        String strahlerName = CopyOfNetworkChannel.STRAHLERNAME;
        b.add(strahlerName, Integer.class);
        b.add(CopyOfNetworkChannel.PFAFNAME, String.class);
        if (inDem != null) {
            b.add(CopyOfNetworkChannel.STARTELEVNAME, Double.class);
            b.add(CopyOfNetworkChannel.ENDELEVNAME, Double.class);
        }
         ...
         * connect channels
         */
        channels = new ArrayList<CopyOfNetworkChannel>();
        for( SimpleFeature network : networkList ) {
            channels.add(new CopyOfNetworkChannel(network));
        }
        pm.beginTask("Connect channels...", channels.size());
        for( CopyOfNetworkChannel channel : channels ) {
            for( CopyOfNetworkChannel checkChannel : channels ) {
         ...
         * calculate netnum
         */
        pm.beginTask("Calculate netnum...", IJGTProgressMonitor.UNKNOWN);
        calculateStrahler();
        pm.done();
    ...
    private void calculatePfafstetter() {
        for( int i = 1; i <= maxHack; i++ ) {
            ...
            List<CopyOfNetworkChannel> startChannels = new ArrayList<CopyOfNetworkChannel>();
            for( CopyOfNetworkChannel channel : channels ) {
                int hack = channel.getHack();
                CopyOfNetworkChannel nextChannel = channel.getNextChannel();
                if (hack == i && (nextChannel == null || nextChannel.getHack() != i)) {
                    startChannels.add(channel);
                }
            }

            for( CopyOfNetworkChannel startChannel : startChannels ) {
            	CopyOfNetworkChannel nextChannel = startChannel.getNextChannel();
                String base = "";
                if (nextChannel != null) {
                ...
                while( startChannel.getPreviousChannels().size() > 0 ) {
                    List<CopyOfNetworkChannel> previousChannels = startChannel.getPreviousChannels();
                    for( CopyOfNetworkChannel copyOfNetworkChannel : previousChannels ) {
                        if (copyOfNetworkChannel.getHack() == i) {
                            startChannel = copyOfNetworkChannel;
                            startChannel.setPfafstetter(base + index);
                            index = index + 2;
                            break;
                        }
    ...
    private void calculateStrahler() {
        // calculate Strahler
        List<CopyOfNetworkChannel> sourceChannels = new ArrayList<CopyOfNetworkChannel>();
        for( CopyOfNetworkChannel channel : channels ) {
            if (channel.isSource()) {
        ...
        List<CopyOfNetworkChannel> nextsList = new ArrayList<CopyOfNetworkChannel>();
        List<CopyOfNetworkChannel> toKeepList = new ArrayList<CopyOfNetworkChannel>();
        while( true ) {
            sourceChannels.addAll(nextsList);
            nextsList.clear();
            sourceChannels.addAll(toKeepList);
            toKeepList.clear();
            for( CopyOfNetworkChannel sourceChannel : sourceChannels ) {
            	CopyOfNetworkChannel nextChannel = sourceChannel.getNextChannel();
                if (nextChannel != null) {
            ...
            for( CopyOfNetworkChannel copyOfNetworkChannel : nextsList ) {
                List<CopyOfNetworkChannel> previousChannels = copyOfNetworkChannel.getPreviousChannels();
                ...
                for( CopyOfNetworkChannel channel : previousChannels ) {
                ...
                if (allEqual) {
                	copyOfNetworkChannel.setStrahler(maxStrahler + 1);
                } else {
                	copyOfNetworkChannel.setStrahler(maxStrahler);
                }
            }
