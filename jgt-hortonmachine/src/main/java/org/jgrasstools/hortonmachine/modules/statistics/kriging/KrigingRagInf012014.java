...
public class KrigingRagInf012014 extends JGTModel {
	...
	@Description("The interpolated gridded data (for mode 2 and 3.")
	@Out
	public GridCoverage2D outVarGrid = null;
	...
	@Description("The interpolated data (for mode 0 and 1).")
	@Out
	public HashMap<Integer, double[]> outVarData = null;
	...
	private WritableRaster outVarWR = null;
	...
	public void executeKriging() throws Exception {
		...
		double[] result_var = new double[pointsToInterpolateId2Coordinates
				.size()];
		...
		while (idIterator.hasNext()) {
			...
			if (n1 != 0) {
				...
				if (!areAllEquals && n1 > 1) {
					...
					double h0 = 0.0;
					double var_h0 = 0.0;
					double zs[] = new double[zStationOK.length - 1];
					double hs[] = new double[zStationOK.length - 1];
					int ids[] = new int[zStationOK.length - 1];
					...
					if (doDetrended) {
						...
						// ///////////PSEUDO-FUNZIA////////////////////////////
						if (plocalTrend == false) {
							for (int k = 0; k < zs.length; k++) {
								if (hStationOK[k] >= 0.0) {
									zs[cp] = zStationOK[k];
									hs[cp] = hStationOK[k];
									ids[cp] = idStationOK[k];

									cp += 1;
								}
							}
						} else {
							zs = new double[zStation.length - 1];
							hs = new double[zStation.length - 1];
							ids = new int[zStation.length - 1];
							for (int k = 0; k < zs.length; k++) {
								if (hStation[k] >= 0.0) {
									zs[cp] = zStation[k];
									hs[cp] = hStation[k];
						...
						if (cp > 3) {
							double zsok[] = new double[cp];
							double hsok[] = new double[cp];
							int idsok[] = new int[cp];
							for (int aaa = 0; aaa < cp; aaa++) {
								zsok[aaa] = zs[aaa];
								hsok[aaa] = hs[aaa];
								idsok[aaa] = ids[aaa];

							}
							r = new Regression(zsok, hsok);
							r.linear();
							...
							if (r.getXYcorrCoeff() > thresholdCorrelation
									&& trend > 0 && cp > 3) {
								double[] hsokres = r.getResiduals();

								// CALCOLO SEMI DEI RESIDUI
								HashMap<Integer, double[]> inDataRes = new HashMap<Integer, double[]>();
								for (int i = 0; i < idsok.length; i++) {
									inDataRes.put(idsok[i],
											new double[] { hsokres[i] });
								}
								Meuse.pm = pm;
								Meuse.inStations = inStations;
								Meuse.fStationsid = fStationsid;
								Meuse.inData = inDataRes;
								Meuse.pCutoff = 15000;
								Meuse.process();
								outDistances = Meuse.outDist;
								outSemivarinces = Meuse.outVar;
								double bestsill = 0;
								double bestnugget = 0;
								double bestraange = 0;
								double minRMSE = 0;
								String bestm = "pippo";
								if (Meuse.outAllEquals == false
										&& Meuse.diversi > 2
										&& Meuse.outVar.length > 2) {

									PS a = new PS();
									if (Meuse.outVar.length == 0) {
									...
									for (int ooo = 0; ooo < vetmodel.length; ooo++) {
										...
										a.ParRange_minn = new double[] { 0.0,
												0, 0 };
										a.ParRange_minn = new double[] { 0.0,
												0.0, mindist };
										a.ParRange_maxn = new double[] { 0.0,
												maxvar * 1, maxdist * 1 };
										a.process();
										// System.out.println(a.g_best_value +
										// " nugget="
										// + a.g_best[0] + " sill=" +
										// a.g_best[1] +
										// " range="
									...
									pNug = bestnugget;
									pS = bestsill;
									pSemivariogramType = bestm;
									System.out
											.println("HO CALCOLATO IL SEMIVARIOGRAMMA DEI RESIDUI:"
													+ " nugget="
													+ bestnugget
													+ " range="
													+ bestraange
													...
													+ bestsill
													+ " model=" + bestm);
									for (int i = 0; i < idStation.length - 1; i++) {
									...
									pSemivariogramType = "linear";
									System.out
											.println("HO USATO IL DEFAULT PER IL SEMIVARIOGRAMMA DEI RESIDUI:"
													+ " nugget="
													+ pNug
													+ " range="
													+ pA
					...
					for (int k = 0; k < n1; k++) {
						...
						var_h0 = var_h0 + moltiplicativeFactor[k]
								* knownTerm[k];

					}
					var_h0 = var_h0 + moltiplicativeFactor[n1];
					if (doDetrended) {
						h0 = h0 + trend;
						var_h0 += trend;
					}
		...
		if (pMode == 0) {
			...
			storeResultVar(result_var, idArray);
		} else {
			storeResult(result, pointsToInterpolateId2Coordinates);
			storeVarResult(result_var, pointsToInterpolateId2Coordinates);
	...
	private void storeResultVar(double[] result2, int[] id)
			throws SchemaException {
		outVarData = new HashMap<Integer, double[]>();
		for (int i = 0; i < result2.length; i++) {
			outVarData.put(id[i], new double[] { result2[i] });
		}
	}
	...
	private void storeResult(double[] interpolatedValues,
		...
		WritableRandomIter outIter = RandomIterFactory.createWritable(outWR,
				null);
	...
	private void storeVarResult(double[] interpolatedValues,
			HashMap<Integer, Coordinate> interpolatedCoordinatesMap)
			throws MismatchedDimensionException, Exception {

		WritableRandomIter outIter = RandomIterFactory.createWritable(outVarWR,
				null);
		...
		outVarGrid = CoverageUtilities.buildCoverage("gridded", outVarWR,
	...
	private LinkedHashMap<Integer, Coordinate> getCoordinate(GridGeometry2D grid) {
		...
		outWR = CoverageUtilities.createDoubleWritableRaster(cols, rows, null,
				null, null);
		outVarWR = CoverageUtilities.createDoubleWritableRaster(cols, rows, null,
				null, null);
