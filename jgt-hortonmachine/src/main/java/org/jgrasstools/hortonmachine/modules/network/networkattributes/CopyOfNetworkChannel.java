 ...
 */
public class CopyOfNetworkChannel {
    ...
    private CopyOfNetworkChannel nextChannel;
    private List<CopyOfNetworkChannel> previousChannels = new ArrayList<CopyOfNetworkChannel>();
    private SimpleFeature currentChannel;

    public CopyOfNetworkChannel( SimpleFeature currentChannel ) {
        this.currentChannel = currentChannel;
    }

    public void setNext( CopyOfNetworkChannel next ) {
    ...
    public CopyOfNetworkChannel getNextChannel() {
        return nextChannel;
    }

    public List<CopyOfNetworkChannel> getPreviousChannels() {
    ...
    public void setNetnum( int value ) {
        currentChannel.setAttribute(NETNUMNAME, value);
    }
    ...
    public int getNetnum() {
        Object attribute = currentChannel.getAttribute(NETNUMNAME);
        if (attribute == null) {
            return -1;
        } else {
            return (Integer) attribute;
    ...
    public void addPrevious( CopyOfNetworkChannel previous ) {
        if (!previousChannels.contains(previous)) {
            previousChannels.add(previous);
        }
    }

    public void checkAndAdd( CopyOfNetworkChannel checkChannel ) {
